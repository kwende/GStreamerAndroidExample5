                   ...
                   gpointer           data)
{
    ...
    {
        __android_log_print(ANDROID_LOG_ERROR, "GOOGLIEBAH", "%s,%s: %s",
...
execute_seek (gint64 desired_position, CustomData * data)
{
    ...
    if (GST_CLOCK_TIME_IS_VALID (data->last_seek_time) && diff < SEEK_MIN_DELAY) {
        ...
        /* Update the desired seek position. If mu ltiple petitions are received before it is time
...
state_changed_cb (GstBus * bus, GstMessage * msg, CustomData * data)
{
    ...
    gchar *message = g_strdup_printf ("GOOGLIEBAH State changed from %s to %s",
...
app_function (void *userdata)
{
    ...
    char* l1 = getenv ("LC_ALL");
    char* l2 = getenv ("LC_CTYPE");
    char* l3 = getenv ("LANG");

    GST_DEBUG ("strings: %s, %s, %s", l1, l2, l3);
...
JNI_OnLoad (JavaVM * vm, void *reserved)
{
    ...
    jclass klass = (*env)->FindClass (env,
                                      "com/ocuvera/tutorial5/MainActivity");
